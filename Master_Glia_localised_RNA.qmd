...
glia_type_dgem_by_gene %>%
  ...
  summarise(n_transcriptome = sum(pct_exp_cell >= 2.5)) %>% ungroup() %>%
...
# INCORPORATE HUMAN DISEASE ASSOCIATIONS

  Incorporate human disease associations with the glial localised transcripts
  Biomart - Ensemble does not have OMIM annotations 
  
  Query Flybase human disease tsv
...
fb_disease <- read_tsv("./data/Flybase/disease_model_annotations_fb_2022_04.tsv", skip = 4) %>%
  clean_names()

find_disease_genes <- function(fb_disease, term){
  fb_disease %>%
    filter(str_detect(do_term, term)) %>%
    filter(!str_detect(gene_symbol, "\\\\")) %>%
    pull(number_number_f_bgn_id) %>%
    unique() %>%
    return()
}

find_disease_terms <- function(fb_disease, term){
  fb_disease %>%
    filter(str_detect(do_term, term)) %>%
    filter(!str_detect(gene_symbol, "\\\\")) %>%
    pull(do_term) %>%
...
find_disease_terms(fb_disease, "lewy")
find_disease_genes(fb_disease, "Huntington's disease")

## Get neurodegen genes 
search_strings <- c("Parkinson's", "Alzheimer", "fragile X syndrome", "Huntington's disease", "amyotrophic", "spinal muscular atrophy") %>% 
  c(paste(search_strings, collapse = "|")) %>%
  set_names(c("Parkinson", "Alzheimer", "FXS", "Huntington", "ALS", "SMA", "Neurodegenerative_total"))

nd_genes_list <- search_strings %>%
  map(~ find_disease_genes(fb_disease, .x))

qsave(nd_genes_list, "./data/Flybase/neurodegenerative_disease_list.qs")
nd_genes_list <- qread("./data/Flybase/neurodegenerative_disease_list.qs")

## Incorporate into the main dataframe
gl_is <- qread('./output/glia-localised-rna_mimic-incorporated.qs')
nd_genes_int <- nd_genes_list %>%
  imap(~{
    gl_is <- gl_is %>%
      mutate(!!.y := if_else(dmel_gene_id %in% .x, TRUE, FALSE)) %>%
      dplyr::select(!!.y)
  }) %>%
  bind_cols()

nd_genes_list %>% map(~ length(.x))

gl_nd <- bind_cols(gl_is, nd_genes_int)
gl_nd %>% qsave("./output/glia-localised-neurodegenerative-disease-incorporated.qs")

## Test for overrepresentation (enrichment)
## phyper(overlap - 1, group2, Total - group2, group1, lower.tail = FALSE)
phyper(90 - 1, 200, 17000 - 200, 1700, lower.tail = FALSE)
...
```{r}
write_tsv(gl_nd, './glia-localised-rna.txt')
```
...
id_interest %>% 
  filter(sfari_dmel == TRUE) %>% nrow()

str <- 'actin|microtubule'

id_interest %>% 
  filter(str_detect(go_molecular_function, str) |
         str_detect(go_biological_process, str) |
         str_detect(go_cellular_component, str)) %>%
...
tmp <- id_interest %>%
  mutate(actin_microtubule_related = if_else(
    str_detect(go_molecular_function, 'actin|microtubule') | 
    str_detect(go_biological_process, 'actin|microtubule') | 
    str_detect(go_cellular_component, 'actin|microtubule'),
    TRUE,
    FALSE
  )) %>%
  mutate(RNA_binding = if_else(
    str_detect(go_molecular_function, 'RNA binding') | 
    str_detect(go_biological_process, 'RNA binding') | 
    str_detect(go_cellular_component, 'RNA binding'),
    TRUE,
    FALSE
  )) %>%
  mutate(ribosome_associated = if_else(
    str_detect(go_molecular_function, 'ribosome') | 
    str_detect(go_biological_process, 'ribosome') | 
    str_detect(go_cellular_component, 'ribosome'),
    TRUE,
    FALSE
  )) %>%
  mutate(locomotorly_regulation = if_else(
    str_detect(go_molecular_function, 'locomotor') | 
    str_detect(go_biological_process, 'locomotor') | 
    str_detect(go_cellular_component, 'locomotor'),
    TRUE,
    FALSE
  )) %>%
  mutate(molecular_motor_activity = if_else(
    str_detect(go_molecular_function, 'motor activity') | 
    str_detect(go_biological_process, 'motor activity') | 
    str_detect(go_cellular_component, 'motor activity'),
    TRUE,
    FALSE
  ))
...
tmp %>%
  dplyr::select(dmel_gene_id, dmel_gene_name, mmus_gene_symbols, translation_present, transcript_enriched, translation_enriched, mimic_or_crimic, sfari_dmel, go_biological_process, go_molecular_function, go_cellular_component, synapse_localised, synapse_organization, synaptic_signalling, actin_microtubule_related, RNA_binding, ribosome_associated, locomotorly_regulation, molecular_motor_activity, Imp_target, Syp_target) %>%
  write_csv('~/Desktop/1700_localised_transcript_with_annotations.csv')

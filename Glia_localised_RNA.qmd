...
```{r}
df_raw_merge %>%
  dplyr::select(-contains("mmus_go")) %>%
  rename_with(~ str_c("dmel_biomart_", .x), contains("go_")) %>%
  rename_with(~ str_c("fb_go_", .x), contains("synap")) %>%
  write_tsv("~/Desktop/glial-localised-RNA_dmel-conversion.txt")
